    ...
    <div className="bg-white">
      ...
      >
        <div className="bg-white bg-opacity-60 p-4 rounded">
          <h1 className="text-4xl font-bold text-gray-800">All Reports</h1>
        </div>
      </div>

      {/* Reports Section */}
      <section className="max-w-7xl mx-auto px-4 py-12">
        <h2 className="text-3xl font-bold text-center mb-10">Explore All Our Reports</h2>

        <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-3">
          {allReports.map((report) => (
            <div
              key={report.id}
              className="bg-gray-50 rounded-lg shadow hover:shadow-lg transition duration-300 overflow-hidden flex flex-col"
            >
              <img
                src={report.image}
                alt={`Cover image of ${report.title}`}
                loading="lazy"
                className="w-full h-48 object-cover"
              />
              <div className="p-5 flex-1 flex flex-col">
                <h3 className="text-xl font-semibold mb-3">{report.title}</h3>
                <p className="text-gray-600 flex-1">{report.description}</p>
              </div>
            </div>
          ))}
        </div>
